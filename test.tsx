...
test('<Flexifit />', t => {
	...
	const iframeProps = { allowFullscreen: true, frameBorder: "0" };

	t.equal((shallow(
		<Flexifit aspectRatio={1.7778}>
			<iframe src="https://www.youtube.com/embed/1g6QJ5TfA7w" {...iframeProps}></iframe>
		</Flexifit>
	).state() as any).ratio, 1.7778, 'Flexifit should accept an `aspectRatio` prop and retain that in state as `ratio`')

	t.equal((shallow(
		<Flexifit width={560} height={315}>
			<iframe src="https://www.youtube.com/embed/1g6QJ5TfA7w" {...iframeProps}></iframe>
		</Flexifit>
	).state() as any).ratio, 560 / 315, 'Flexifit should accept `height` and `width` props and calculate the aspect ratio from that')
